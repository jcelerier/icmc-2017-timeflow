...
\capstarttrue
\begin{abstract}
    Numeric musical and artistic creation is in great part based on well-known patcher software: Max/MSP, PureData, OpenMusic, vvvvv\dots 
...
\end{tikzpicture}
\caption{The matching temporal graph}
\label{fig.tgraph}
\end{figure}
...
We define also define a strictness level for the delayed connection. 
...
\node (t3) at (6.5, -3.45) {\footnotesize $t_2$};
\end{tikzpicture}
\caption{Another execution trace. $f_2$'s input corresponds to $f_1$'s output, delayed: this creates a causality problem.}
\label{fig.delay}
\end{figure}
...
\begin{figure}[h]
    ...
    \begin{subfigure}{0.21\textwidth}\centering
        ...
        \caption{$f_1, f'_1$: generates a random note on each bang}
    \end{subfigure}
    \begin{subfigure}{0.21\textwidth}\centering
        \includegraphics[scale=0.5]{images/pd-2.png}
        \caption{$f_2$: generates another random note on each bang}
    \end{subfigure}
    
    \begin{subfigure}{0.21\textwidth}\centering
        \includegraphics[scale=0.5]{images/pd-3.png}
        \caption{$f_3$: synthesizes a note, sets the volume}
    \end{subfigure}
    \begin{subfigure}{0.21\textwidth}\centering
        \includegraphics[scale=0.5]{images/pd-4.png}
        \caption{$f_4$: applies an high-pass filter}
    \end{subfigure}
    \begin{subfigure}{0.21\textwidth}\centering
        \includegraphics[scale=0.5]{images/pd-5.png}
        \caption{$f_5, f'_5$: applies a low-pass filter}
    \end{subfigure}
    \caption{The functions we use in our example, as defined in PureData.}
    \label{fig.pdpatches}
\end{figure}
...
\begin{figure}[h]
    ...
    \caption{The data relationships between instances of $f_{1,..5}$ in the program.}
    \label{fig.datapatch}
\end{figure}
...
\begin{enumerate}
    ...
    \item $f_2$ starts: random notes will be generated and put at the \lstinline|/note| input of $f_3$ directly, bypassing the environment.
    ...
    \item $f'_1$ starts: tones are generated again, but with a faster rhythm.
